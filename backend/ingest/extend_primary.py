...
def load_fill_df(fill_path: Path) -> pd.DataFrame:
    ...
    if df_fill.empty:
        return pd.DataFrame(
            columns=["ticker", "_ticker_upper", "exchange", "gurufocus_ticker", "gurufocus_exchange"]
        )
...
def enrich_flattened_df_with_primary_listing(
    ...
    *,
    ...
    """
    Enrich df_flat with gurufocus_ticker and gurufocus_exchange.
    ...
    Resolution priority (highest → lowest):
      ...
      4. ticker itself as fallback for gurufocus_ticker
      5. "UNKNOWN" as fallback for gurufocus_exchange

    extra_overrides: list of {ticker, gurufocus_ticker, gurufocus_exchange} dicts.
    """
    ...
    if "gurufocus_ticker" not in df.columns:
        df["gurufocus_ticker"] = pd.Series([None] * len(df), dtype="string")
    else:
        df["gurufocus_ticker"] = df["gurufocus_ticker"].astype("string")

    if "gurufocus_exchange" not in df.columns:
        df["gurufocus_exchange"] = pd.Series([None] * len(df), dtype="string")
    else:
        df["gurufocus_exchange"] = df["gurufocus_exchange"].astype("string")
    ...
    if extra_overrides:
        ...
        for row in extra_overrides:
            ...
            extra_rows.append({
                ...
                "gurufocus_ticker": _norm_str(row.get("gurufocus_ticker")),
                "gurufocus_exchange": _norm_str(row.get("gurufocus_exchange")),
            })
        if extra_rows:
    ...
    if not df_fill.empty:
        df_fill2 = df_fill.rename(
            columns={
                ...
                "gurufocus_ticker": "gurufocus_ticker_fill",
                "gurufocus_exchange": "gurufocus_exchange_fill",
            }
        )
        df = df.merge(
            df_fill2[["_ticker_upper", "exchange_fill", "gurufocus_ticker_fill", "gurufocus_exchange_fill"]],
            on="_ticker_upper",
            how="left",
        )
    else:
        df["exchange_fill"] = pd.Series([None] * len(df), dtype="string")
        df["gurufocus_ticker_fill"] = pd.Series([None] * len(df), dtype="string")
        df["gurufocus_exchange_fill"] = pd.Series([None] * len(df), dtype="string")
    ...
    df["gurufocus_ticker"] = (
        df["gurufocus_ticker_fill"]
        .combine_first(df["gurufocus_ticker"])
        .combine_first(_ticker_normalized)
        .astype("string")
    )
    df["gurufocus_exchange"] = (
        df["gurufocus_exchange_fill"]
        .combine_first(df["exchange_fill"])
        .combine_first(df["gurufocus_exchange"])
    ...
    df = df.drop(
        columns=["_ticker_upper", "exchange_fill", "gurufocus_ticker_fill", "gurufocus_exchange_fill"]
    )

    # Normalize Nordic share class tickers: "NOVO.B" -> "NOVO B"
    from ingest.resolve_tickers import _normalize_ticker_for_gurufocus
    df["gurufocus_ticker"] = df.apply(
        lambda row: _normalize_ticker_for_gurufocus(
            str(row["gurufocus_ticker"]), str(row["gurufocus_exchange"])
        ),
        axis=1,
    ).astype("string")
