...
export default function LongEquityInsight({ snapshots }: { snapshots: Snapshot[] }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/longequity/latest-available`)
      .then((r) => r.json())
      .then((d) => {
        if (d.available) setLatestAvailable(`${MONTH_NAMES[d.month]} ${d.year}`);
        else setLatestAvailable('Not found');
      })
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between gap-6">
        <div>
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <button
            onClick={runIngest}
            disabled={ingesting}
            className="px-3 py-1 rounded text-xs font-mono bg-gray-700 hover:bg-gray-600 disabled:opacity-50 text-white transition-colors whitespace-nowrap"
          >
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {ingestLog.length > 0 && (
          <div className="bg-gray-950 border border-gray-800 rounded p-2 max-h-64 overflow-y-auto font-mono text-xs space-y-0.5 mb-4">
