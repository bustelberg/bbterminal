...
/**
 * S&P 500 import / inspect / freeze tool.
 *
 * Single-snapshot model (2026-06): a universe is one frozen set as of a
 * date, not a per-month series — so this page imports the current S&P 500
 * constituents, shows that single set, and freezes a reusable copy. The old
 * month-scrubber / changelog / cumulative views (month-over-month history)
 * were dropped along with per-month membership.
 */
export default function IndexUniverse() {
  ...
  // Freeze a static, pipeline-immune copy of the selected index's current set.
  // The snapshot then appears wherever frozen universes are listed (/universe,
  // /backtest, /earnings).
  const freezeCopy = useCallback(async () => {
    if (!selectedIndex || freezing) return;
    setFreezing(true);
    setFreezeMsg(null);
    try {
      const r = await apiFetch(
        `${API_URL}/api/index-universe/freeze?index=${encodeURIComponent(selectedIndex)}`,
        { method: 'POST' },
      );
  ...
  const filteredTickers = useMemo(() => (
    tickerFilter
      ? tickers.filter(t =>
          ...
          (t.exchange || '').toLowerCase().includes(tickerFilter.toLowerCase())
        )
      : tickers
      ...
      <div className="shrink-0 px-8 py-5 border-b border-neutral-800/40">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-fg-subtle text-sm mt-0.5">Current S&amp;P 500 constituents — import the latest set, inspect it, and freeze reusable copies.</p>
          </div>
          <button
            onClick={runImport}
            disabled={running}
                ...
                {indexes.map(idx => (
                  ...
                  >
                    <div>
                      ...
                      <span className="text-fg-subtle text-xs ml-3">as of {idx.end_month}</span>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-xs text-fg-muted">{idx.total_unique_tickers} tickers</span>
                      <button
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="min-w-0">
                  ...
                  <p className="text-fg-subtle text-xs mt-0.5">
                    Create a static, pipeline-immune snapshot of {selectedIndex}
                    {snapshotMonth ? ` as of ${snapshotMonth}` : ''}. It becomes selectable in /universe, /backtest and /earnings.
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    title={`Freeze ${selectedIndex}'s current constituents as a reusable universe`}
                  >
                    {freezing ? 'Freezing…' : 'Freeze copy'}
                  </button>
                </div>
              </div>
              {freezeMsg && (
            ...
            {(gfLogs.length > 0 || gfResult) && (
              <div className="bg-card rounded-xl border border-neutral-800/40">
                ...
                <div className="p-5 space-y-4">
                  {gfLogs.length > 0 && !gfResult && (
                    <ProgressTimeline steps={[]} log={gfLogs} running={checkingGF} defaultLogOpen />
                  )}
                  {gfResult && (
