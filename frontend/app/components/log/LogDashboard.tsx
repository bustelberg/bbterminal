    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <section className={`${card} xl:h-[clamp(420px,calc(100vh-14rem),560px)] xl:overflow-y-auto`}>
          ...
          <label className="mt-3 block text-sm text-fg-soft">{nl ? 'Evalueren op' : 'Review on'}<input type="date" value={form.review_on} onChange={e => setForm({ ...form, review_on: e.target.value })} className="mt-1 block rounded-lg border border-neutral-700 bg-page px-3 py-2 text-fg" /></label>
