...
/** The base inputs behind the FCF-SBC yield — THREE rows per company (Free Cash Flow, SBC, Market
 * cap), each in the company's own reporting currency (millions). Same columns / Unsubscribed /
 * Fetch behaviour as the other drill-downs. Self-fetches (so Fetch can reload). Mirrors
 * {@link ./MarginInputsModal}. */

const LINES: { key: 'fcf' | 'sbc' | 'market_cap'; label: string; muted?: boolean }[] = [
  { key: 'fcf', label: 'Free Cash Flow' },
  { key: 'sbc', label: 'SBC', muted: true },
  { key: 'market_cap', label: 'Market cap', muted: true },
];
...
export default function FcfSbcYieldInputsModal({ target, portfolioName, onClose }: {
  ...
  const rows = useMemo(() => {
    const get: Record<SortKey, (r: FcfSbcYieldRow) => number | string | null | undefined> = {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
          <p className="text-[11px] text-fg-faint">Free Cash Flow, SBC and market cap as reported (millions, native currency). Yield = (FCF − SBC) ÷ Market cap.</p>
                    ...
                    return (
                      <Fragment key={r.isin}>
                        ...
                        <tr className="hover:bg-overlay/[0.02]">
                          ...
                          <td className="px-3 py-1 whitespace-nowrap text-fg-soft font-medium">FCF-SBC yield</td>
                          {years.map((y) => (
                            <td key={y} className="px-3 py-1 text-right font-mono text-fg-soft font-medium">
                              {fmtRatioPct(fcfSbcYieldOf(r.fcf[y], r.sbc[y], r.market_cap[y]))}
                            </td>
                          ))}
                        </tr>
                      </Fragment>
