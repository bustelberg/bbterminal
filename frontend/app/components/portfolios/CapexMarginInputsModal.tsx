...
const LINES: InputsLine<CapexMarginRow>[] = [
  { label: 'Capex', of: (r, y) => r.capex[y] },
  { label: 'Revenue', of: (r, y) => r.revenue[y], muted: true },
];

export default function CapexMarginInputsModal({ target, portfolioName, benchTarget, benchLabel, onClose }: {
  ...
  const load = async (body: Target | BenchTarget): Promise<CapexMarginInputs> => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        console.warn('[bb:bench] capex-margin-inputs constituents:', e);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-5">
          <p className="text-[12px] text-fg-faint">Capex (as reported — negative = outflow) and revenue (millions, native currency). Margin = |Capex| ÷ Revenue.</p>
