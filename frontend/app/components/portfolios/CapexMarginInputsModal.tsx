...
import { capexMarginOf, type CapexMarginInputs, type CapexMarginRow } from './capexMarginData';
...
const LINES: { key: 'capex' | 'revenue'; label: string; muted?: boolean }[] = [
...
export default function CapexMarginInputsModal({ target, portfolioName, onClose }: {
  ...
  const rows = useMemo(() => {
    const get: Record<SortKey, (r: CapexMarginRow) => number | string | null | undefined> = {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex items-baseline gap-3 px-6 py-4 border-b border-neutral-800/40">
          <h2 className="text-fg-strong font-medium">Holdings — Capex-margin inputs by year</h2>
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
          <p className="text-[11px] text-fg-faint">Capex (as reported — negative = outflow) and revenue (millions, native currency). Margin = |Capex| ÷ Revenue.</p>
                    ...
                    return (
                      <Fragment key={r.isin}>
                        ...
                        <tr className="hover:bg-overlay/[0.02]">
                          ...
                          <td className="px-3 py-1 whitespace-nowrap text-fg-soft font-medium">Capex margin</td>
                          {years.map((y) => (
                            <td key={y} className="px-3 py-1 text-right font-mono text-fg-soft font-medium">
                              {fmtRatioPct(capexMarginOf(r.capex[y], r.revenue[y]))}
                            </td>
                          ))}
                        </tr>
                      </Fragment>
