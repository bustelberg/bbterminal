...
const en = {
  ...
  columnGroups: {
    return: { label: 'How the AIRS return is built', hint: '(Value now − Beginwaarde) + Realised + Income = Result, ÷ Beginwaarde' },
    onmoney: { label: 'How money-weighted return is built', hint: 'dated XIRR, shown cumulatively' },
  ...
  info: {
    ...
    moneyAggregateNote: 'A money-weighted aggregate: the covered positions’ combined result divided by their combined average invested capital. Individual position rows use dated XIRR.',
  ...
  row: {
    ...
    heldDirectly: 'held directly', atOpen: 'at the open',
    ...
    airsReturnNote: (valuedBy?: string) => `${valuedBy ? `${valuedBy}: ` : ''}AIRS price and currency result plus net dividends attributable to the remaining shares; later sales can reduce this dividend leg, while later purchases never increase it`,
    airsReturnHow: '(AIRS price result + AIRS currency result + net dividend × min(1, remaining shares ÷ shares on payment date)) ÷ AIRS Beginwaarde',
  ...
  classRow: {
    ...
    coverageTitle: (pct: string, bucket: string, missing: number) => `This rate covers ${pct}% of ${bucket}'s eligible opening value. ${missing} position(s) had an opening value but no AIRS return.`,
    ...
    returnHow: (result: string, opening: string, pct: string) => `Σ (each position’s AIRS return × its Beginwaarde) ÷ Σ Beginwaarde\n\n${result} ÷ ${opening} = ${pct}`,
    returnNote: (result: string, opening: string, coverage?: string) => `${result} weighted AIRS result ÷ ${opening}${coverage ? ` · covers ${coverage}% of eligible opening value` : ''}`,
    contributionNote: (result: string) => `${result} ÷ the book’s opening capital`,
    contributionHow: (result: string, basis: string, contribution: string) => `Result ÷ the book’s opening capital\n\n${result} ÷ ${basis} = ${contribution}`,
...
const nl: AnalyseCopy = {
  ...
  columnGroups: {
    return: { label: 'Opbouw AIRS-rendement', hint: '(Huidige waarde − Beginwaarde) + Gerealiseerd + Inkomsten = Resultaat, ÷ Beginwaarde' },
    onmoney: { label: 'Opbouw geldgewogen rendement', hint: 'XIRR op datums, cumulatief getoond' },
  ...
  info: {
    ...
    moneyAggregateNote: 'Een geldgewogen totaal: het gezamenlijke resultaat van de gedekte posities gedeeld door hun gezamenlijke gemiddeld belegde kapitaal. Afzonderlijke positieregels gebruiken XIRR op datums.',
  ...
  row: {
    ...
    heldDirectly: 'rechtstreeks aangehouden', atOpen: 'bij aanvang',
    ...
    airsReturnNote: (valuedBy) => `${valuedBy ? `${valuedBy}: ` : ''}AIRS koers- en valutaresultaat plus nettodividend dat aan de resterende aandelen toekomt; latere verkopen kunnen dit dividenddeel verlagen, latere aankopen verhogen het nooit`,
    airsReturnHow: '(AIRS koersresultaat + AIRS valutaresultaat + nettodividend × min(1, resterende aandelen ÷ aandelen op betaaldatum)) ÷ AIRS Beginwaarde',
  ...
  classRow: {
    ...
    coverageTitle: (pct, bucket, missing) => `Dit rendement dekt ${pct}% van de meetbare beginwaarde van ${bucket}. ${missing} positie(s) hadden wel een beginwaarde maar geen AIRS-rendement.`,
    ...
    returnHow: (result, opening, pct) => `Σ (AIRS-rendement per positie × Beginwaarde) ÷ Σ Beginwaarde\n\n${result} ÷ ${opening} = ${pct}`,
    returnNote: (result, opening, coverage) => `${result} gewogen AIRS-resultaat ÷ ${opening}${coverage ? ` · dekt ${coverage}% van de meetbare beginwaarde` : ''}`,
    contributionNote: (result) => `${result} ÷ beginkapitaal van het boek`,
    contributionHow: (result, basis, contribution) => `Resultaat ÷ beginkapitaal van het boek\n\n${result} ÷ ${basis} = ${contribution}`,
