...
import { yieldOf, type BASIS, type Rebased, type YearPoint } from './quickValuation';

/**
 * The data behind the Price-vs-earning-power chart: the two raw lines as reported, then the two
 ...
 * FCF / EPS the chart was showing when it was opened, not decide for itself.
 */
export default function QuickValuationInputsModal({
  points, index, currency, basis: b, name, isin, onClose,
}: {
  points: YearPoint[];
  index: Rebased;
          ...
          <div className="overflow-auto rounded-lg border border-neutral-800/40">
            <table className="w-full text-xs">
              <thead className="bg-page">
                <tr className="text-fg-faint text-xs uppercase tracking-wide border-b border-neutral-800/40">
              ...
              <tbody>
                ...
                <tr className="hover:bg-overlay/[0.02]">
                  ...
                  {points.map((p) => (
                    // A cash-burn or loss year is a real observation, marked so it doesn't read as
                    // a small positive at a glance — it is the year the index rebase worked around.
                    <td key={p.year}
                      className={`px-3 py-1 text-right font-mono ${p.value != null && p.value < 0 ? 'text-neg-300' : 'text-fg-soft'}`}>
                      {num(p.value)}
                    </td>
                  ))}
                </tr>
                ...
                <tr className="border-t border-neutral-800/40 hover:bg-overlay/[0.02]">
                  ...
                  {points.map((p) => {
                    const y = yieldOf(p.value, p.price);
                    return (
                      <td key={p.year}
