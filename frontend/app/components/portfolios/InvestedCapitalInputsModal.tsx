...
/** The base inputs behind Invested capital — the two components (Non-current liabilities, Total
 * equity) plus their sum (Invested capital), each in the company's own reporting currency
 * (millions). Reuses `cash-return-inputs` (which returns both legs). Same columns / Unsubscribed /
...
// Invested capital for a year = NCL + total equity (both present), else null.
const investedOf = (r: CashReturnRow, y: string) => {
  const ncl = r.noncurrent_liabilities[y];
  const eq = r.total_equity[y];
  return ncl == null || eq == null ? null : ncl + eq;
};

export default function InvestedCapitalInputsModal({ target, portfolioName, onClose }: {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex items-baseline gap-3 px-6 py-4 border-b border-neutral-800/40">
          <h2 className="text-fg-strong font-medium">Holdings — invested-capital inputs by year</h2>
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
          <p className="text-[11px] text-fg-faint">Non-current liabilities and total equity as reported (millions, native currency). Invested capital = non-current liabilities + total equity.</p>
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {rows.map((r) => {
                    ...
                    // Two raw components + their sum; the sum row is the plotted level.
                    return (
                      <Fragment key={r.isin}>
                        <tr className="border-t border-neutral-800/40 hover:bg-overlay/[0.02]">
                          {head}
                          <td className="px-3 py-1 whitespace-nowrap text-fg-muted">Non-curr. liabilities</td>
                          {years.map((y) => <td key={y} className="px-3 py-1 text-right font-mono text-fg-soft">{fmtRevM(r.noncurrent_liabilities[y])}</td>)}
                        </tr>
                        <tr className="hover:bg-overlay/[0.02]">
                          <td className="px-3 py-1 sticky left-0 bg-card z-10" /><td /><td /><td /><td />
                          <td className="px-3 py-1 whitespace-nowrap text-fg-muted">Total equity</td>
                          {years.map((y) => <td key={y} className="px-3 py-1 text-right font-mono text-fg-soft">{fmtRevM(r.total_equity[y])}</td>)}
                        </tr>
                        <tr className="hover:bg-overlay/[0.02]">
                          <td className="px-3 py-1 sticky left-0 bg-card z-10" /><td /><td /><td /><td />
                          <td className="px-3 py-1 whitespace-nowrap text-fg-soft font-medium">Invested capital</td>
                          {years.map((y) => <td key={y} className="px-3 py-1 text-right font-mono text-fg-soft font-medium">{fmtRevM(investedOf(r, y))}</td>)}
                        </tr>
                      </Fragment>
                    );
                  })}
