...
const LINES: { key: 'fcf' | 'net_income'; label: string; muted?: boolean }[] = [
  { key: 'fcf', label: 'Free cash flow' },
  { key: 'net_income', label: 'Net income', muted: true },
];
...
export default function CashConversionInputsModal({ target, portfolioName, onClose }: {
  ...
  const rows = useMemo(() => {
    const get: Record<SortKey, (r: CashConversionRow) => number | string | null | undefined> = {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
          <p className="text-[11px] text-fg-faint">Free cash flow and net income (millions, native currency). Conversion = FCF ÷ Net Income. Above 100% is normal — depreciation ahead of capex. A loss has no conversion, so the percentage is blank there.</p>
                    ...
                    return (
                      <Fragment key={r.isin}>
                        ...
                        <tr className="hover:bg-overlay/[0.02]">
                          ...
                          <td className="px-3 py-1 whitespace-nowrap text-fg-soft font-medium">FCF / Net Income</td>
                          {years.map((y) => (
                            <td key={y} className="px-3 py-1 text-right font-mono text-fg-soft font-medium">
                              {fmtRatioPct(cashConversionOf(r.fcf[y], r.net_income[y]))}
                            </td>
                          ))}
                        </tr>
                      </Fragment>
