 ...
 * Operating cash flow), each in the company's own reporting currency (millions). Same columns /
 * Unsubscribed / Fetch behaviour as the other drill-downs. Self-fetches (so Fetch can reload).
 * Mirrors {@link ./InterestBurdenInputsModal}. */
...
const LINES: InputsLine<SbcOcfRow>[] = [
  { label: 'Stock-based comp.', of: (r, y) => r.sbc[y] },
  { label: 'Operating cash flow', of: (r, y) => r.ocf[y], muted: true },
];

export default function SbcOcfInputsModal({ target, portfolioName, benchTarget, benchLabel, onClose }: {
  ...
  const load = async (body: Target | BenchTarget): Promise<SbcOcfInputs> => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        console.warn('[bb:bench] sbc-ocf-inputs constituents:', e);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex items-baseline gap-3 px-6 py-4 border-b border-neutral-800/40">
          <h2 className="text-fg-strong font-medium">Holdings — SBC / OCF inputs by year</h2>
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-5">
          <p className="text-[12px] text-fg-faint">Stock-based compensation and operating cash flow as reported (millions, native currency). Ratio = SBC ÷ Operating cash flow.</p>
