...
export default function MarginCard({ holdingsTarget, holdingsName }: {
  ...
  const marginByYr = useMemo(() => marginByYear(data?.rows ?? []), [data]);
  ...
  return (
    <div className="rounded-xl border border-neutral-800/40 bg-card p-4 space-y-3 min-w-0">
      <h4 className="text-base font-semibold text-fg-strong">FCF-SBC margin</h4>
          ...
          <div className="flex flex-wrap gap-2">
            <Stat label="Avg margin" value={pct(avg)} color={chartTheme.accent}
              info={<InfoTip content={<AspectCard
                what="Average FCF-SBC margin over the years shown."
