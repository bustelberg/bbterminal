...
export default function GrossMarginCard({ holdingsTarget, holdingsName }: {
  ...
  return (
    <div className="rounded-xl border border-neutral-800/40 bg-card p-4 space-y-3 min-w-0">
      ...
      {data == null && !err ? (
        ...
        <p className="text-[11px] text-fg-faint py-16 text-center">No gross-profit / revenue figures ingested to compute a margin.</p>
      ) : (
        <>
          ...
          <div>
            <ResponsiveContainer width="100%" height={320}>
              <ComposedChart data={chartData} margin={{ top: 5, right: 12, bottom: 5, left: 4 }}
                ...
                <Tooltip contentStyle={chartTheme.tooltipCard.contentStyle} labelStyle={{ color: chartTheme.axisLabel }}
                  formatter={(v) => [`${typeof v === 'number' ? v.toFixed(1) : '—'}%`, 'Gross margin']} />
            ...
            <div className="flex justify-center flex-wrap gap-x-4 gap-y-1 text-xs mt-1">
              <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 inline-block rounded" style={{ background: chartTheme.accent }} />Gross margin (avg dashed)</span>
            </div>
          </div>
        </>
      )}
