...
/** The base inputs behind the Gross margin — TWO rows per company (Gross Profit, Revenue), each
 * in the company's own reporting currency (millions).
 *
 * ⚠ A BLANK GROSS PROFIT IS AN ANSWER. A bank has no such line at all (no cost of goods sold), so
 * the row is empty and the margin is blank — not 0, which would read as selling at cost.
...
const LINES: { key: 'gross_profit' | 'revenue'; label: string; muted?: boolean }[] = [
  { key: 'gross_profit', label: 'Gross profit' },
  { key: 'revenue', label: 'Revenue', muted: true },
];
...
export default function GrossMarginInputsModal({ target, portfolioName, onClose }: {
  ...
  const rows = useMemo(() => {
    const get: Record<SortKey, (r: GrossMarginRow) => number | string | null | undefined> = {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex items-baseline gap-3 px-6 py-4 border-b border-neutral-800/40">
          <h2 className="text-fg-strong font-medium">Holdings — Gross-margin inputs by year</h2>
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-3">
          <p className="text-[11px] text-fg-faint">Gross profit and revenue (millions, native currency). Margin = Gross Profit ÷ Revenue. A blank gross profit is a bank — no cost of goods sold, so no gross margin exists.</p>
                    ...
                    return (
                      <Fragment key={r.isin}>
                        ...
                        <tr className="hover:bg-overlay/[0.02]">
                          ...
                          <td className="px-3 py-1 whitespace-nowrap text-fg-soft font-medium">Gross margin</td>
                          {years.map((y) => (
                            <td key={y} className="px-3 py-1 text-right font-mono text-fg-soft font-medium">
                              {fmtRatioPct(grossMarginOf(r.gross_profit[y], r.revenue[y]))}
                            </td>
                          ))}
                        </tr>
                      </Fragment>
