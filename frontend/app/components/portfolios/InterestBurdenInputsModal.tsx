...
const LINES: InputsLine<InterestBurdenRow>[] = [
  { label: 'Interest expense', of: (r, y) => r.interest_expense[y] },
  { label: 'Operating income', of: (r, y) => r.operating_income[y], muted: true },
];

export default function InterestBurdenInputsModal({ target, portfolioName, benchTarget, benchLabel, onClose }: {
  ...
  const load = async (body: Target | BenchTarget): Promise<InterestBurdenInputs> => {
    ...
    return b as InterestBurdenInputs;
  };
  ...
  const [bench, setBench] = useState<InterestBurdenInputs | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        console.warn('[bb:bench] interest-burden-inputs constituents:', e);
