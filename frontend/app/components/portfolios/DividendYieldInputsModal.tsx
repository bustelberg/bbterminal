...
const LINES: InputsLine<DividendYieldRow>[] = [
  { label: 'Dividends / share', of: (r, y) => r.div_ps[y] },
  { label: 'Year-end price', of: (r, y) => r.price_ps[y], muted: true },
];

export default function DividendYieldInputsModal({ target, portfolioName, benchTarget, benchLabel, onClose }: {
  ...
  const load = async (body: Target | BenchTarget): Promise<DividendYieldInputs> => {
    ...
    return b as DividendYieldInputs;
  };
  ...
  const [bench, setBench] = useState<DividendYieldInputs | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        console.warn('[bb:bench] dividend-yield-inputs constituents:', e);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-scrim/60 p-4"
      ...
      <div className="bg-card rounded-xl border border-neutral-800/40 shadow-xl w-[88vw] h-[84vh] flex flex-col"
        ...
        <div className="flex-1 overflow-auto px-6 py-4 space-y-5">
          <p className="text-[11px] text-fg-faint">Dividends per share and the fiscal year-end price as reported (native currency, per share). Yield = Dividends / share ÷ Year-end price. A blank dividend is “not ingested”, a 0 is “pays nothing”.</p>
          ...
          <div className="space-y-1.5">
            ...
            {data && data.rows.length > 0 && (
              <RatioInputsTable data={data} lines={LINES} derived={derived} fmtValue={fmtPs} onFetch={fetchFinancials} />
            )}
          </div>
            ...
            <div className="space-y-1.5">
              ...
              {bench && (
                <>
                  ...
                  <RatioInputsTable data={bench} lines={LINES} derived={derived} fmtValue={fmtPs} />
                </>
              )}
            </div>
          )}
