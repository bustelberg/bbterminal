...
export default function MarginInputsModal({ target, portfolioName, onClose }: {
  ...
  const rows = useMemo(() => {
    const get: Record<SortKey, (r: MarginRow) => number | string | null | undefined> = {
          ...
          {data && data.rows.length > 0 && (
            <div className="overflow-auto rounded-lg border border-neutral-800/40">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {rows.map((r) => {
                    // Company-level cells (rendered on the first line of the company's block).
                    const head = (
                      <>
