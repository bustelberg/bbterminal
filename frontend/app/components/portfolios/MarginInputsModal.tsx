...
const LINES: InputsLine<MarginRow>[] = [
  { label: 'Revenue', of: (r, y) => r.revenue[y] },
  { label: 'FCF', of: (r, y) => r.fcf[y] },
...
export default function MarginInputsModal({ target, portfolioName, benchTarget, benchLabel, onClose }: {
  ...
  const load = async (body: Target | BenchTarget): Promise<MarginInputs> => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        console.warn('[bb:bench] margin-inputs constituents:', e);
