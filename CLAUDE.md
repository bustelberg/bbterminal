...
**Entry point**: `main.py` (~110 lines) — thin bootstrap: constructs `FastAPI()`, attaches CORS + the admin-only-mutations middleware, mounts each domain router, registers APScheduler. All endpoints live under `routers/`. Full endpoint list: `main.py` + `backend/openapi.json` (the source of truth — don't catalog endpoints here).
...
Modern fintech terminal, dark by default with a runtime light theme. **Everything routes through `@theme` design tokens in `frontend/app/globals.css`** — never inline hex or raw Tailwind colour names (`gray-*`, `white`, `indigo-*`, …). Codemods tokenized ~250 surface literals + ~1,000 named-colour classes + ~2,000 grey/white classes; new code uses the token utilities below.
- **Surfaces** (7): `bg-page` / `bg-card` / `bg-sidebar` / `bg-card-alt` (derived-universe tint) / `bg-elevated` / `bg-popover` / `bg-inset`.
- **Semantic ramps**: `accent-*` (brand — buttons, active nav, links, focus rings), `pos-*` (positive returns / success), `neg-*` (negative returns / errors), `warn-*` (warnings / out-of-scope). Each aliases a Tailwind ramp (`accent`→indigo, `pos`→emerald, `neg`→rose, `warn`→amber) — repoint one `var()` to re-skin (e.g. `--color-accent-500: var(--color-violet-500)`).
- **Text**: `text-fg-strong` (white headings) · `text-fg` (body) · `text-fg-soft`/`-muted`/`-subtle`/`-faint`/`-dim` (decreasing emphasis). **Neutral** (borders/dividers/control surfaces): `border-neutral-800` (default), `-700`/`-600`/`-500` etc. **Washes**: `hover:bg-overlay/5` (hover/active wash, white→black in light) · `bg-scrim/60` (modal backdrop). Opacity modifiers work everywhere (`bg-page/40`, `bg-pos-500/15`, `border-neutral-800/40`) and are runtime-safe (Tailwind v4 emits `color-mix(var())` under `@supports`).
- **Theming**: dark = the token defaults on `:root`; **light = the override block `:root[data-theme="light"]`** (surfaces + fg/neutral ramps invert, accent/return *text* shades darken for contrast, wash flips to black). A no-FOUC `next/script` in `app/layout.tsx` applies the stored choice (`localStorage['bb-theme']`) before paint; `ThemeToggle` (sidebar footer) flips it via `useSyncExternalStore` over the `<html data-theme>` attribute. **Style presets** (orthogonal): `data-radius="sharp|round"` + `data-density="compact|comfortable"` on `<html>` reshape corner radius + overall scale (em-based) with zero component changes. Charts are the one exception — `lib/chartTheme.ts` is still dark-tuned (a known follow-up to make it read CSS vars).
...
### In-process pipeline (`backend/scheduler.py`)

Two `BackgroundScheduler` triggers fire the SAME five-phase pipeline — **no Railway Cron entries**:
- `weekly_price_volume` — Tuesday 02:00 UTC (captures prior Monday's global closes; US settles ~5h earlier).
- `monthly_price_volume` — 2nd of month 02:00 UTC (first trading day's closes; weekend/holiday 1st short-circuits, weekly catches up).

Each fire calls `kick_off_refresh(job_name, "auto")` (`routers/ingest_runs.py`) → inserts an `ingest_run` (`triggered_by='auto'`) → spawns `_run_pipeline_sync` (`ingest/phases/pipeline.py`):
- **Phase 0 — acquisition**: LongEquity (ingest only if upstream month is newer), ACWI iShares XLS (warn if mtime > 14d — iShares blocks automated download so the XLS is committed to the repo manually), Leonteq (no-op; refresh hits its API in Phase 1). Failures → `error_summary`, don't abort.
- **Phase 1 — template refresh**: each `UniverseTemplate.refresh()` ensures its `universe` row, reconstructs monthly memberships from `earliest_date`, produces a `TemplateDiff`. Results → `templates_summary`; per-template failures isolated (`error` field).
- **Phase 2 — orphan prune**: `prune_orphan_companies` keeps the union of LongEquity / ACWI / LEONTEQ memberships (+ `metric_data.source_code='longequity'` legacy fallback), deletes the rest (cascades `metric_data` + `portfolio_weight`). Runs before prices so deleted rows don't waste GuruFocus calls.
- **Phase 3 — price + volume refresh**: `ensure_prices_for_company` + `ensure_volume_for_company` over every `company`; counters checkpoint every 100; forbidden/delisted/error tallies on the row.
- **Phase 4 — momentum compute**: each enabled `scheduled_strategy` drains `_momentum_backtest_stream` (`mode=current_portfolio`), persists one `current_picks_snapshot` (with `ingest_run_id` + `backtest_run_id`). Per-strategy isolated → `momentum_summary`; any failure marks the run `error` but successful snapshots still persist.

Phases are independent (a failure is captured but the next still runs); `current_phase` = live phase or `done`. Only aggregates land on `ingest_run` (not per-company); first ~5 unexpected errors → `error_summary`.

**Trade-offs / gotchas**:
- ✅ Code-managed (deploys with `git push`; cron expressions in `scheduler.py`).
- ⚠️ Single-instance assumption — N replicas each fire a tick (freshness checks no-op duplicates: wasteful, not broken). Set `DISABLE_SCHEDULER=1` on all but one if you scale out (also the pause switch).
- ⚠️ A restart landing exactly at 02:00 UTC drops that tick; next week catches up.
- The `POST /api/ingest/scheduled-refresh/cron` HTTP endpoint still exists (X-Cron-Secret) for reverting to Railway Cron without code changes — unused today.
