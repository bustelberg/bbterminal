...
Single fixed theme: **"Glass Institutional (tempered)"** — frosted-navy chrome (sidebar + floating menus) over a gradient-mesh background, *solid* data surfaces so dense tables/numbers stay crisp, electric blue→cyan accent with a soft button glow. No theme switcher (user-facing or otherwise) — the whole palette + treatment lives in `frontend/app/globals.css`'s `@theme` block + the few "Glass treatment" rules under it. **Everything routes through these `@theme` design tokens** — never inline hex or raw Tailwind colour names (`gray-*`, `white`, `indigo-*`, …). Codemods tokenized ~250 surface literals + ~1,000 named-colour classes + ~2,000 grey/white classes. To re-skin, edit token values in `globals.css`; nothing else.
- **Surfaces** (7): `bg-page` (navy base) / `bg-sidebar` + `bg-popover` (translucent → frosted by the `backdrop-filter` rule) / `bg-card` + `bg-card-alt` (derived-universe tint) + `bg-elevated` + `bg-inset` (all **solid**). `bg-card/-card-alt/-elevated` get a top-highlight + drop-shadow for glassy depth; `body` carries the radial gradient mesh.
