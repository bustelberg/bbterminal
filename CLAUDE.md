...
- **Frozen-only policy (2026-06-15)**: universes are listed frozen-snapshots-only EVERYWHERE selectable/browsable — `/backtest` + `/regime-detector` (useStaticUniverses), `/earnings` (`/api/earnings/universes`), `/universe` screener (`/api/universe/labels` filtered `frozen_at IS NOT NULL`), `/companies` memberships. The ONLY exception is `/longequity-universe`'s own per-month view. The management pages (`/acwi`, `/sp500`, `/leonteq`) still show their LIVE universe as the freeze *source* (you freeze a copy from there).
...
- **`/earnings` (`EarningsDashboard.tsx`)** has ONE Stock/Portfolio switch (`aMode`; side B follows via `const bMode = aMode`):
  ...
  - **Portfolio mode**: shows ONLY the **Allocation × Selection matrix** (`AttributionMatrix`) comparing two **baskets** — saved portfolios and/or frozen-universe snapshots → `GET /api/earnings/portfolios/attribution?a=&b=&a_kind=&b_kind=&universe=&year=` (`_basket_members` dispatches portfolio vs universe; chunk all `.in_()` over the union — two large universes overflow a single URI). No charts/snapshot; the matrix has its own price-only load.
...
- **Decomposed god-components (2026-06-04)** — each is now a thin orchestrator (≤~115 lines) over a `app/components/<x>/` subfolder following the same hooks+subcomponents pattern. **When extending these pages, add/extend a hook or section component — don't regrow the orchestrator.**
  ...
  - `CompanyManager.tsx` (115) → `company/`: `useCompanies` (data + Add/Edit/Delete mutations + options) + `useCompanyFilters` (search/filter/sort) hooks; `MultiSelectFilter`, `CompaniesToolbar`, `CompanyTable` (+ `AddRow`/`EditRow`/`CompanyRow`), `VerifyAddModal`. Covered by `/companies` e2e; verify with `npm run e2e`. **Sector** column shows the company's sector + a coloured **source-universe** annotation (`/api/companies/sectors` → `{cid:{sector,source}}`, backed by `company_sector_with_source` RPC: Leonteq-preferred, else latest-month, live-before-frozen). **Memberships** chips (incl. frozen universes) + the sector annotation get one **distinct colour per universe** via `buildUniverseStyles` (even hue spread by sorted label — replaces the old per-label hash that clustered). `hideUnlinked` filter (default ON) hides companies in no universe (orphaned index stubs); header shows the count + toggle. **Name correction from GuruFocus** (fixes mislabeled listings, e.g. a row stored "TSMC" whose `TSE:2330` listing GuruFocus calls "Forside Co Ltd"): per-row **"GF name"** action (`POST /api/admin/gurufocus-company-name` → confirm → PUT) and in bulk via **Refresh market caps** (the market-cap backfill now also reads `summary.company_data.company` and overwrites `company_name` on a real, punctuation-insensitive mismatch — `index_universe/backfill_market_cap.py`). **Mkt Cap** stores + shows the native value, currency, and ECB FX rate behind the EUR figure (`company.market_cap_{native,currency,fx_rate}`; foreign-currency line + audit tooltip). An empty cap on an exchange outside the GuruFocus subscription (AU/NZ, Russia, Africa, LatAm — `is_gf_subscribed_exchange` over `FEASIBLE_GF_EXCHANGES`) shows an **UNSUBSCRIBED** badge (`gf_unsubscribed` in `/api/companies`, computed in BOTH the PostgREST and COPY paths — `momentum/data/_pg.py::load_companies_via_copy`).
