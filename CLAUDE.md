...
**`tests/`** — pytest, momentum engine only (`test_signals`, `test_scoring`, five `test_backtest_*`). Ingest/template/prune/routers have no unit tests — covered at HTTP level by `backend-stack-smoke`. Run `uv run pytest tests/`.
...
- **Colors**: bg `#0f1117` (page) / `#151821` (cards) / `#0b0d13` (sidebar); accent `indigo-600` buttons, `indigo-400` active nav; returns `emerald-400` pos / `rose-400` neg; text `white` headings, `gray-200` body, `gray-400/500` secondary.
- **Type**: Geist Sans for UI; Geist Mono only for numeric values.
- **Components**: cards `bg-[#151821] rounded-xl border border-gray-800/40`; tables in card containers, `text-sm`, `py-2.5` rows; primary button `bg-indigo-600 hover:bg-indigo-500`, ghost `hover:bg-white/5`; inputs `bg-[#0f1117] border border-gray-700 rounded-lg focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30`; row hover `hover:bg-white/[0.02]`, actions fade in `opacity-0 group-hover:opacity-100`; errors `bg-rose-500/10 border border-rose-500/20 rounded-lg`.
...
- **Momentum signal cutoff is strict `<`** (`signals.py`) — never train on the bar we trade; companies with last trade > 30 days before `as_of_date` are dropped.
...
Each fire calls `kick_off_refresh(job_name, "auto")` (`routers/ingest_runs.py`) → inserts an `ingest_run` (`triggered_by='auto'`) → spawns `_run_pipeline_sync`:
...
- God-components to keep shrinking: `MomentumBacktester.tsx` (~670 lines, config + variants now decomposed and the current-picks UI removed; remaining bulk = saved-runs handlers + `loadBacktest`/`saveVariantsBundle`), `AcwiUniverse.tsx` (~1,080), `CompanyManager.tsx` (~1,050), `Schedule.tsx` (~1,020).
...
- Backend unit tests are momentum-only; `backend-stack-smoke` gives HTTP-level coverage (incl. an end-to-end backtest assertion) but non-momentum routers + ingest/template/admin paths are only smoke-probed for 2xx.
