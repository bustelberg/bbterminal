...
**`tests/`** — pytest, momentum engine only (`test_signals`, `test_scoring`, five `test_backtest_*`). Ingest/template/prune/routers have no unit tests — covered at HTTP level by `backend-stack-smoke`. (The `ingest/phases/` split now makes each pipeline phase unit-testable against a fake Supabase client — a good place to start closing that gap.) Run `uv run pytest tests/`.
